...
fn load_recursive(
    ...
    for (alias, directory) in parsed.dependencies {
        let target_manifest = directory.join("staple.kdl");
...
mod tests {
    ...
    fn loads_recursive_renamed_dependencies() {
        ...
        std::fs::write(
            workspace.join("app/staple.kdl"),
            "package \"app\" { dependencies { renamed path=\"../middle\" } }\n",
        )
        .unwrap();
        std::fs::write(
            workspace.join("middle/staple.kdl"),
            "package \"middle-lib\" { kind \"library\"; dependencies { leaf path=\"../leaf\" } }\n",
        )
        .unwrap();
        std::fs::write(
            workspace.join("leaf/staple.kdl"),
            "package \"leaf\" { kind \"library\" }\n",
        )
        .unwrap();
        let graph = load_package_graph(&workspace.join("app/staple.kdl")).unwrap();
    ...
    fn rejects_dependency_cycles_and_non_library_targets() {
        ...
        std::fs::write(
            workspace.join("a/staple.kdl"),
            "package \"a\" { dependencies { b path=\"../b\" } }\n",
        )
        .unwrap();
        std::fs::write(workspace.join("b/staple.kdl"), "package \"b\"\n").unwrap();
        assert!(
            load_package_graph(&workspace.join("a/staple.kdl"))
                .unwrap_err()
                .contains("must have kind \"library\"")
        );
        std::fs::write(
            workspace.join("b/staple.kdl"),
