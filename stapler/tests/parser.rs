...
#[test]
fn parses_effect_sets_declarations_and_handlers_losslessly() {
    let source = concat!(
        "pub effect State = S => {\n",
        "  get: () -> S\n",
        "  set: S -> ()\n",
        "}\n",
        "def increment: () ->{State I32, State I32} I32 = () => {\n",
        "  let value = State.get ()\n",
        "  State.set (value + 1)\n",
        "  value\n",
        "}\n",
        "def InitialState: Handler (State I32) {} = handler State I32 {\n",
        "  get () => resume 10\n",
        "  set value => resume ()\n",
        "}\n",
        ...
        "}\n",
        "def named = handle increment () with InitialState\n",
    );
    let module = parse(source).expect("effect syntax should parse");
    assert_eq!(module.text(), source);
    let Item::EffectDeclaration(effect) = &module.items[0] else {
        panic!("expected effect declaration")
    };
    assert_eq!(effect.name, "State");
    assert_eq!(effect.type_parameters.len(), 1);
    assert_eq!(effect.operations.len(), 2);
    let Statement::Binding(increment) = statement(&module.items[1]) else {
        panic!("expected increment binding")
    };
    let Type::Function(function) = increment.annotation.as_ref().unwrap() else {
        panic!("expected effectful function type")
    };
    assert_eq!(function.effects.effects.len(), 2);
    let Statement::Binding(handler) = statement(&module.items[2]) else {
        panic!("expected handler binding")
    };
    assert!(matches!(handler.annotation, Some(Type::Handler(_))));
    assert!(matches!(handler.value, Some(Expression::Handler(_))));
    let Statement::Binding(result) = statement(&module.items[3]) else {
        panic!("expected result binding")
    };
    assert!(matches!(result.value, Some(Expression::Handle(_))));
    let Statement::Binding(named) = statement(&module.items[4]) else {
        panic!("expected named result binding")
    };
    assert!(matches!(named.value, Some(Expression::Handle(_))));
}

#[test]
fn parses_independent_curried_effect_sets_and_empty_sets() {
    let source = "def curried: A ->{Console} B ->{} C = a => b => b\n";
    let module = parse(source).expect("curried effect sets should parse");
    let Statement::Binding(binding) = statement(&module.items[0]) else {
        panic!()
    };
    let Type::Function(first) = binding.annotation.as_ref().unwrap() else {
        panic!()
    };
    assert_eq!(first.effects.effects.len(), 1);
    let Type::Function(second) = first.result.as_ref() else {
        panic!()
    };
    assert!(second.effects.effects.is_empty());
    assert_eq!(module.text(), source);
}

#[test]
fn requires_effect_sets_on_handler_types() {
    let error = parse("let value: Handler Console\n")
        .expect_err("handler types require an explicit clause-effect set");
