...
An effect declares a family of dynamically dispatched operations. Effects may
be generic, and every operation must have a function type whose own arrow is
effect-free:
...
An operation is a first-class function value. Calling it contributes its owning
concrete effect to the current expression. Both `State.get` and an unqualified
`get` refer to the same operation when the latter is unambiguous; qualification
is required when several visible effects declare that name.

Function arrows carry independent effect sets. A plain arrow has the empty set,
while `A ->{E1} B ->{E2} C` performs `E1` in the first call and `E2` in the
second. Sets are flattened, unordered, duplicate-free, and printed in a stable
canonical order. An explicit set is an upper bound retained in the public type;
when no function annotation is present, effects are inferred through recursive
calls and ordinary control flow.

`handle` dynamically routes one complete effect to the nearest handler. A
manual handler must contain every operation exactly once and cannot mix effects:
...
The handled computation and every clause share the handle expression's result
type. `resume value` supplies the selected operation's result and continues the
suspended computation under the same handler (a deep handler). Clause code
itself runs outside the matched handler, so performing the same operation there
routes outward. A resumption is affine and may be consumed at most once on any
control-flow path. A manual clause may omit `resume` to abort its continuation.

A reusable handler is an ordinary first-class value, implicitly polymorphic
over the preserved computation result. `Handler E {F}` records the single
handled effect `E` and an upper bound `F` on effects performed directly by its
clauses. The braces are required even when that set is empty:

```staple
def StdConsole: Handler Console {PlatformIO} =
    handler Console {
        print message => {
            platform.stdout_write message
            resume ()
        }
        read_line () => resume platform.stdin_read_line ()
    }

handle program () with StdConsole
```

The operand after `with` is an expression, so handlers can be passed, returned,
stored, selected, captured, imported, and re-exported like other values. It is
evaluated before its fresh dynamic frame is installed, and applying it borrows
rather than consumes it. Reusable clauses must resume or diverge on every
reachable path; unlike manual clauses, they cannot abort or manufacture a
result. Handler targets may mention enclosing type parameters and specialize to
concrete effects at use sites.

Handling removes only the selected concrete effect; effects performed directly
by clauses and all other effects propagate outward. Executable top-level
initialization and compile-time macro execution must have no residual effects.
