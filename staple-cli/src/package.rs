...
pub fn run(arguments: impl IntoIterator<Item = OsString>) -> Result<Outcome, String> {
    ...
    if matches!(arguments.as_slice(), [command, argument] if (argument == "-h" || argument == "--help") && matches!(command.to_str(), Some("build" | "check" | "new" | "run")))
    {
        return Ok(Outcome::Completed(Some(format!(
            "{}\n",
            command_usage(arguments[0].to_str().unwrap())
        ))));
    }
    ...
    // The compiler runs in-process: `compiler_argv` builds the low-level argument
    // vector that the `staple compile`/`staple check` front-end accepts, and we
    // hand it straight to `compile::run` rather than spawning a child process.
    if package.entry.is_none() {
        return compile::run(compiler_argv(&package, &options, None));
    }

    match options.mode {
        Mode::Check => compile::run(compiler_argv(&package, &options, None)),
        ...
        Mode::Build | Mode::Run => {
            ...
            // A compile failure comes back as `Err` and propagates to `main`.
            compile::run(compiler_argv(&package, &options, Some(&output)))?;
...
fn parse_options(arguments: impl IntoIterator<Item = OsString>) -> Result<Options, String> {
    ...
    if mode == Mode::New {
        let name = arguments
            ...
            .into_string()
            .map_err(|_| "`staple new` requires a UTF-8 package name".to_owned())?;
    ...
    while let Some(argument) = arguments.next() {
        if argument == "--" {
            if mode != Mode::Run {
                return Err(format!(
                    "`--` is only supported by `staple run`\n{}",
                    mode_usage(mode)
                ));
            }
...
fn discover_manifest(explicit: Option<&Path>, current: &Path) -> Result<PathBuf, String> {
    ...
    loop {
        let candidate = directory.join("staple.kdl");
        if candidate.is_file() {
            return canonical_file(&candidate, "manifest");
        }
        if !directory.pop() {
            return Err(
                "could not find `staple.kdl` in the current directory or any parent directory"
                    .to_owned(),
            );
        }
    }
...
fn validate_package_name(name: &str) -> Result<(), String> {
    staple_project::validate_package_name(name)
}
...
fn compiler_argv(package: &Package, options: &Options, output: Option<&Path>) -> Vec<OsString> {
...
fn usage() -> String {
    concat!(
        "usage: staple <build|check|new|run> [options]\n",
        "\n",
        "commands:\n",
...
fn command_usage(command: &str) -> String {
    match command {
        "build" => concat!(
            "usage: staple build [options]\n\n",
            "  --manifest-path <path>  use a specific staple.kdl\n",
        ...
        "new" => concat!(
            "usage: staple new <name>\n\n",
            "Create a new package in a directory named <name>.",
        ),
        "run" => concat!(
            "usage: staple run [options] [-- <arguments>...]\n\n",
            "  --manifest-path <path>  use a specific staple.kdl\n",
            "  --linker <command>       linker driver\n",
            "  --stdlib <path>          Staple standard-library root\n",
            "  --features <names>       enable package features\n",
...
mod tests {
    ...
    impl Fixture {
        fn new() -> Self {
            ...
            let root = std::env::temp_dir().join(format!(
                "staple-package-test-{}-{nonce}-{sequence}",
                std::process::id()
            ));
        ...
        fn manifest(&self, source: &str) -> PathBuf {
            let path = self.root.join("staple.kdl");
    ...
    fn parses_new_with_one_safe_package_name() {
        ...
        assert!(command_usage("new").starts_with("usage: staple new <name>"));
    }
    ...
    fn creates_a_loadable_project_with_exact_scaffold_contents() {
        ...
        assert_eq!(
            std::fs::read_to_string(destination.join("staple.kdl")).unwrap(),
            "package \"hello-world\"\n"
        );
        assert_eq!(
        ...
        let package =
            load_package(&destination.join("staple.kdl")).expect("generated manifest should load");
    ...
    #[test]
    fn constructs_exact_compiler_commands() {
        let fixture = Fixture::new();
        ...
        let arguments = compiler_argv(&package, &options, Some(&output));
    ...
    fn pure_library_commands_validate_without_an_artifact() {
        ...
        assert_eq!(compiler_argv(&package, &build, None)[0], "check");
