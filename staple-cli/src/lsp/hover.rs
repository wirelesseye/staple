...
impl Collector<'_> {
    ...
    fn collect_expression_declarations(&mut self, expression: &Expression) {
        match expression {
            ...
            Expression::Unary(unary) => {
                self.collect_expression_declarations(&unary.operand);
            }
        ...
        for (name, module, file_backed) in chain {
            ...
            if let Some(ty) = resolved.companion_type_for_module(module) {
                if let Some(signature) = self.type_signature(ty, declaration.syntax.id) {
                    ...
                    self.named_with_docs(&declaration.syntax, &name, signature, docs, module_label);
                }
            } else if !file_backed {
    ...
    fn imported_module_segments(&mut self, declaration: &UseDeclaration, kind: &UseKind) {
        ...
        for index in usize::from(rooted)..components.len() {
            ...
            let module_label = segment_module.and_then(|module| self.module_id_label(module.id));
    ...
    fn imported_name(
        ...
        for (signature, docs) in signatures {
            ...
            if last {
                self.named_last_with_docs(&declaration.syntax, token_name, signature, docs, module);
            } else {
    ...
    fn expression(&mut self, expression: &Expression) {
            ...
            .compile_time_binding_for(expression.syntax().id)
        {
            ...
            match expression {
                Expression::Name(name) => self.named_with_instantiation(
                    &name.syntax,
                    &name.name,
                    signature,
                    instantiation,
        ...
        if let Some(value_type) = expression_type.as_ref() {
            ...
            let module = self.module_label(expression.syntax().id).or_else(|| {
                self.typed
                    ...
                    .and_then(|trait_id| {
                        self.definition_module_label(DefinitionId::Trait(trait_id))
                    })
            });
    ...
    fn named_last_with_docs(
        ...
        if let Some(range) =
            crate::lsp::source_projection::named_range(syntax, name, true, self.path)
        {
            self.entries.push(HoverEntry {
                range,
                signature,
...
mod tests {
    ...
    fn formats_singleton_type_declarations_without_a_representation() {
        ...
        assert!(
            !entries
                .iter()
                .any(|entry| entry.signature.contains("Marker = "))
        );
    }
    ...
    fn formats_compile_time_lets_and_syntax_constructors_like_source_declarations() {
        ...
        let path = PathBuf::from(env!("CARGO_MANIFEST_DIR"))
            .parent()
            .unwrap()
            .join("staple-compiler/examples/macros.sta");
        let source = std::fs::read_to_string(&path).unwrap();
        let program = ProgramLoader::new()
            .with_standard_library_root(stdlib)
            .load_source_at(&path, &source)
