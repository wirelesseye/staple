...
use crate::lsp::hover::HoverEntry;
use crate::lsp::semantic::SemanticEntry;
...
fn package_config(path: &Path) -> Option<staple_project::PackageGraph> {
    let mut directory = path.parent()?;
    loop {
        let manifest = directory.join("staple.kdl");
        if manifest.is_file() {
            return staple_project::load_package_graph(&manifest).ok();
        }
...
pub fn run(arguments: Vec<std::ffi::OsString>) -> ExitCode {
    if let Err(error) = serve(arguments) {
        eprintln!("staple: {error}");
        return ExitCode::FAILURE;
    }
    ExitCode::SUCCESS
}

fn serve(arguments: Vec<std::ffi::OsString>) -> Result<(), String> {
    let stdlib = parse_options(arguments)?;
                        ...
                        match TypeChecker::new().check(resolved) {
                            Ok(typed) => {
                                ...
                                let completion_index =
                                    crate::lsp::completion::index(surface, &typed);
...
mod tests {
    ...
    fn analyzes_an_empty_open_package_root_against_its_own_source() {
        ...
        std::fs::write(root.join("staple.kdl"), "package \"example\"\n").unwrap();
    ...
    fn hover_and_semantics_track_a_macro_call_in_a_non_first_package_module() {
        ...
        std::fs::create_dir_all(root.join("src")).unwrap();
        std::fs::write(root.join("staple.kdl"), "package \"hello_world\"\n").unwrap();
