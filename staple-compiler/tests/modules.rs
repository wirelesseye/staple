...
impl Fixture {
    fn new() -> Self {
        ...
        let root = std::env::temp_dir().join(format!(
            "staple-compiler-modules-{}-{nonce}-{sequence}",
            std::process::id()
        ));
...
#[test]
fn resolves_recursive_package_dependencies_with_per_package_aliases() {
    let fixture = Fixture::new();
    ...
    .unwrap();
    fs::write(
        fixture.root.join("middle/staple.kdl"),
        "package \"middle-lib\" { kind \"library\"; dependencies { renamed_leaf path=\"../leaf\" } }\n",
    )
    .unwrap();
    fs::write(
        fixture.root.join("leaf/staple.kdl"),
...
#[test]
fn resolves_package_visible_items_within_a_package() {
    let fixture = Fixture::new();
    fixture.write(
        "src/main.sta",
...
#[test]
fn rejects_package_visibility_without_a_manifest() {
    let fixture = Fixture::new();
...
#[test]
fn standard_library_imports_resolve_inside_a_package() {
    let fixture = Fixture::new();
...
fn rejects_an_explicit_std_dependency_alias() {
    ...
    assert!(staple_project::validate_dependency_alias("std").is_err());
}

#[test]
fn manifest_features_filter_items_before_resolution() {
    let fixture = Fixture::new();
    fixture.write(
        "src/main.sta",
